    ...
    internal class ZlibBaseStream : Stream
    {
                ...
                if (this._streamMode == StreamMode.Writer)
                {
                    ...
                    do
                    {
                        ...
                        if ((num != 1) && (num != 0))
                        {
                            ...
                            {
                                throw new ZlibException(string.Format(CultureInfo.InvariantCulture, "{0}: (rc = {1})", str, num));
                            }
                ...
                else if ((this._streamMode == StreamMode.Reader) && (this._flavor == ZlibStreamFlavor.GZIP))
                {
                    ...
                    if (this._z.TotalBytesOut != 0L)
                    {
                        ...
                        if (this._z.AvailableBytesIn < 8)
                        {
                            ...
                            if (count != num5)
                            {
                                throw new ZlibException(string.Format(CultureInfo.InvariantCulture, "Missing or incomplete GZIP trailer. Expected 8 bytes, got {0}.", this._z.AvailableBytesIn + num5));
                            }
                        }
                        else
                        {
                        ...
                        if (num7 != num6)
                        {
                            throw new ZlibException(string.Format(CultureInfo.InvariantCulture, "Bad CRC32 in GZIP trailer. (actual({0:X8})!=expected({1:X8}))", num7, num6));
                        }
                        if (num9 != num8)
                        {
                            throw new ZlibException(string.Format(CultureInfo.InvariantCulture, "Bad size in GZIP trailer. (actual({0})!=expected({1}))", num9, num8));
                        }
                    }
                }
            }
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            do
            {
                ...
                if ((num != 0) && (num != 1))
                {
                    throw new ZlibException(string.Format(CultureInfo.InvariantCulture, "{0}flating:  rc={1}  msg={2}", this._wantCompress ? "de" : "in", num, this._z.Message));
                }
            }
            ...
            if (this._z.AvailableBytesOut > 0)
            {
                ...
                if (this.nomoreinput && this._wantCompress)
                {
                    ...
                    if ((num != 0) && (num != 1))
                    {
                        throw new ZlibException(string.Format(CultureInfo.InvariantCulture, "Deflating:  rc={0}  msg={1}", num, this._z.Message));
                    }
                }
            }
