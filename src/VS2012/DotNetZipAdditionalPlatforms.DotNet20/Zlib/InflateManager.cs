        ...
        Label_07D4:
            switch (this.mode)
            {
                case InflateManagerMode.METHOD:
                    if (this._codec.AvailableBytesIn != 0)
                    {
                        ...
                        if (((this.method = this._codec.InputBuffer[this._codec.NextIn++]) & 15) != 8)
                        {
                            ...
                            this._codec.Message = string.Format(CultureInfo.InvariantCulture, "unknown compression method (0x{0:X2})", this.method);
                            this.marker = 5;
                        }
                        else if (((this.method >> 4) + 8) > this.wbits)
                        {
                            this.mode = InflateManagerMode.BAD;
                            this._codec.Message = string.Format(CultureInfo.InvariantCulture, "invalid window size ({0})", (this.method >> 4) + 8);
                            this.marker = 5;
                        }
                        else
                        {
                ...
                case InflateManagerMode.BAD:
                    throw new ZlibException(string.Format(CultureInfo.InvariantCulture, "Bad state ({0})", this._codec.Message));
            }
